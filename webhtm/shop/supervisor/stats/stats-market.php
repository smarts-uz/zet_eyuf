...
<div id="page">
    ...
    $data = Az::$app->calls->stats->getOrderCountByMarket();
    ...
    if (!empty($post)) {
        ...
        if (!empty($beginDate) && !empty($endDate)) {
            ...
            $data = Az::$app->calls->stats->getOrderCountByMarket($beginDate, $endDate);
            $this->urlRefresh();
    ...
    if ($session) {
        $data = Az::$app->calls->stats->getOrderCountByMarket($session[0], $session[1]);
    }


    $model = new EyufStatsOrdersForm();
    $app = new ALLApp();
    $column = new Form();
