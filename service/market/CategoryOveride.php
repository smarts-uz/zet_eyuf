 ...
 */
class CategoryOveride extends ZFrame
{
    public $categories;
    public $brands;
    ...
    public function generateDBMenuItems($company_id = null)
    {
        ...
        $this->categories = ShopCategory::find()->indexBy('id')
            ->asArray()
            ->all();
        $categories = ImmArray::fromArray($this->categories);

        $this->brands = ShopBrand::find()->indexBy('id')
            ->asArray()
            ->all();
        $brands = ImmArray::fromArray($this->brands);
