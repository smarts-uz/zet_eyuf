...
class AssetManager extends Component
{
    ...
    public function getPublishedPath($path)
    {
        ...
        if (is_string($path) && ($path = realpath($path)) !== false) {
            return $this->basePath . DIRECTORY_SEPARATOR . $this->hash($path) . (is_file($path) ? DIRECTORY_SEPARATOR . bname($path) : '');
        }
    ...
    public function getPublishedUrl($path)
    {
        ...
        if (is_string($path) && ($path = realpath($path)) !== false) {
            return $this->baseUrl . '/' . $this->hash($path) . (is_file($path) ? '/' . bname($path) : '');
        }
