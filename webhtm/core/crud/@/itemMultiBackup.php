...
<div id="page" class="col-md-12">
    ...
    $relation_ids = ZArrayHelper::getValue($this->httpGet(), 'ids');
    ...
    if (!empty($multi)) {
        foreach ($multi as $parametr => $relation) {
            ...
            if ($relation_ids !== null || !empty($relation_ids)) {
                ...
                $relation_ids = $parametr;
                $key = $relation . $relation_ids;
                if ($key !== 'id') {
                    $key = $relation . '[' . $relation_ids . '][]';
