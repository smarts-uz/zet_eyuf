...
function message($items,$market_id)
{
    //vdd($items);
...
HTML;
    ...
    foreach ($items as $item) {
        ...
        if (count($item->items) < 1) {
            ...
            $children .= message($item->items,$market_id);
            $children .= '</div>';
        } else {
...
//vdd($items);
$result = message($items,$market_id);
...
echo $this->require( '/webhtm/shop/user/item-review/modal/review-reply.php', [
