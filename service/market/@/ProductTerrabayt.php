...
class ProductTerrabayt extends ZFrame
{
    /**
...
#region productItem and database
    ...
    public function product($shop_product_id = null, $company_id = null)
    {
        ...
        if ($core_category === null)
            throw new ErrorException("$id product's category is not exist");
        ...
        $item->measure = $measure->_measure[$shop_product['measure']] ?? $measure->_measure['pcs'];
        ...
        foreach ($category_properties as $key1 => $category_property) {
            ...
            foreach ($category_property->items as $key => $value) {
                if ($shop_product['shop_option_ids'] !== null && $key !== null && $shop_product['shop_option_ids'] !== '')
                    if (in_array($key, json_decode($shop_product['shop_option_ids'], true)))
                        $items[$key] = $value;
            }
    ...
    public function propertsByCategory($category_id = null, $is_combination = false, $branch_id = null)
    {
        ...
        foreach ($core_option_types as $core_option_type) {
            ...
            $branch = $this->core_option_branches->where('id', $core_option_type['shop_option_branch_id'])->first();
    ...
    public function is_multi($core_category)
    {
        ...
        $a = false;
        if (empty($option_type_json))
            return false;
        foreach ($option_type_json as $key => $option_type) {
            if (array_keys_exist('is_combination', $key))
                $a = true;
        }
        return $a;
    }
...
#region order and shipment
    ...
    public function itemToForm($item)
    {
        /*  if (is_array($item))
