...
class OrderNurbek extends ZFrame
{
     ...
     */
    public function ordersToCourier($shopShipmentId)
    {
        $shopShipmentId = ShopShipment::findOne($shopShipmentId);
        if ($shopShipmentId)
            return null;
            
        return ShopOrder::find()
            ->where(['shop_shipment_id' => $shopShipmentId->id])
            ->one();
    }
    ...
    public function saveOrders($order)
    {
        ...
        foreach ($products as $product) {
            $id_list[] = (string)$product->company_id;
        }
    ...
    /**
     *
     * @sqlExample   SELECT * FROM shop_order WHERE "id" = 1783
     * @sqlExample   SELECT amount FROM shop_order_item WHERE "shop_order_id" = 1783
     */
    ...
    public function saveOrderFromCpaApi($user_name, $phone_number, $catalog_id, $amount, $cpas_track_id = null)
    {
    ...
    public function beforeSaveOrder(ShopOrder $model)
    {
        ...
        foreach ($items as $item) {
            ...
            if ($catalog->shop_element_id === null) continue;
    ...
    public function changeStatus(ShopOrder $order)
    {
        if ($order->isNewRecord)
            return null;

        if ($order->status_callcenter !== ShopOrder::status_callcenter['approved'])
            return null;

        if ($order->status_callcenter === $order->oldAttributes['status_callcenter'])
            return null;
