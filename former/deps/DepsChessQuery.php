...
/**
 *
 * Class DepsChessQuery
 */
class DepsChessQuery extends ZModel
{
        ...
        return ZArrayHelper::merge(parent::column(), [
            ...
            'attr' => function (Form $column) {
                ...
                $column->data = function ($modelThis) {
                    $th = new ZView();
                    $post = $th->httpPost();
                    $id = ZArrayHelper::getValue($post, 'key');
                    $modelName = ZArrayHelper::getValue($post, 'modelName');
                    ...
                    if ($model === null)
                        return [];
                    if (empty($model->models))
                        return [];
                    $depend_model_class = $th->bootFull($model->models);
                    /** @var ZActiveRecord $depend */
                    $depend = new $depend_model_class();
                    $data = [];
                    foreach ($depend->columns as $attribute => $column){
                        $data[$attribute] = $column->title;
                    }
