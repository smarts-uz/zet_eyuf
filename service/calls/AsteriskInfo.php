...
class AsteriskInfo extends ZFrame
{
    #region Vars
    public ?SFTP$sftp;
    private $path = [
    ...
    public function getRecordFileByName($name)
    {
        if (!empty($this->absolutePath($name))) {
            ...
            $localPath = Root . '/upload/audioz/'.App.'/call/' . $this->absolutePath($name) . '/' . $name;
            $localPath1 = Root . '/upload/audioz/'.App.'/call/' . $this->absolutePath($name) . '/';
