...
class Elfinder2 extends ZFrame
{
    ...
    public function access($attr, $path, $data, $volume, $isDir, $relpath)
    {
        ...
        if ($attr === 'hidden') {
            if (preg_match('/acceptcourier_able.php(?:/|$)/', $path)) {
                return true;
            }
        }
    ...
    function hideCheck($cmd, &$result, $args, $elfinder, $volume)
    {

        if (!empty($result['files'])) {
            foreach ($result['files'] as $i => $file) {
                if ($file['name'] === 'hidden') {
                    unset($result['files'][$i]);
                }
            }
            $result['files'] = array_values($result['files']);
        }
        if (!empty($result['tree'])) {
            foreach ($result['tree'] as $i => $file) {
                if ($file['name'] === 'hidden') {
                    unset($result['tree'][$i]);
                }
            }
            $result['tree'] = array_values($result['tree']);
        }
    }
    ...
    public function layout()
    {
        ...
        $this->_layout['item'] = [
            ...
            'defaults' => ['read' => true, 'write' => true], //Default file/directory permissions. Setting hidden, locked here - take no effect
            ...
            'accessControlData' => "attr", //Data that will be passet to access control method
    ...
    public function run()
    {
        ...
        foreach ($this->data as $item) {
            ...
            $root['accessControl'] = $item->accessControl;
            $root['startPath'] = $item->startPath;
        ...
        $this->options = [
            ...
            //Bind callbacks for user actions, similar to jQuery .bind() More information in documentation
