...
class ProductUMID extends ZFrame
{
...
#region productItem and database
    ...
    {
           /**Тогилаш кере**/
        $data = $this->productByStatus($category_id = null, $status = '', $count = 3, $company_id = null);
    ...
    {
        $data = $this->productsIfHasCatalog();
        ...
        $items = [
            ...
            "text_lang" => null,
            "image" => null,
            "shop_category_id" => 1381,
            "shop_option_ids" => ["1", "3", "4", "5", "7"],
            "shop_brand_id" => null,
            "related" => ["2"],
            "best_before" => null,
            "shelf_life_unit" => "",
    ...
    public function allCompaniesTest($page = null, $limit = null)
    {
        $data = $this->allCompanies();
        $count = count($data);

        ZTest::assertEquals($data, $count);
    }
    ...
    public function allCompanies($page = null, $limit = null)
    {
        ...
        foreach ($companies as $company) {
            ...
            if (\Dash\count($company['photo']) > 0) {
                ...
                // '/upload/uploaz/eyuf/UserCompany/photo/' . $company['id'] . '/' .
                if (file_exists($path))
    ...
    public function ProductTest($shop_product_id = null, $company_id = null, $is_full = false)
    {
        $data = $this->product();
        $item = null;

        ZTest::assertEquals($data,$item);
    }
    ...
    public function product($shop_product_id = null, $company_id = null, $is_full = false)
    {
        ...
        //properties
        if ($is_full) {
            $item->properties = $this->prepertyItems($shop_product['id'], true, true, false);
    ...
    public function testProductwithBranch()
    {
        $data = $this->productWithBranch();
        vd($data);
    }
        ...
        if (!empty($shop_product['image']) || $shop_product['image'] !== null) {
            ...
            if (is_array($imageslist))
                foreach ($imageslist as $image) {
                    $path = '/upload/uploaz/' . App . '/ShopProduct/image/' . $shop_product['id'] . '/' . $image;
    ...
    private function testProductOffers()
    {
    ...
    public function testProductItemByCatalogId()
    {
        $data = $this->productItemByCatalogId();
        vd($data);
    }
    ...
    public function testSingleProductItemByOptions()
    {
        $data = $this->singleProductItemByOptions();
        vd($data);
    }
    ...
    //search uchun
    public function testProductItemByElements()
    {
        $data = $this->productItemByElements();
        vd($data);
    }
    ...
    public function testProductItemByElementId()
    {
        $data = $this->productItemByElementId();
        vd($data);
    }
     ...
     * @return  array
     */
    public function testPropertsByCategory()
    {
        $data = $this->propertsByCategory();
        vd($data);
    }
    ...
    {
        $data = $this->offerCategoriesWithProducts();
        vd($data);
    }
