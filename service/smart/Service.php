...
#region Service
    ...
    public $' . lcfirst($smartClass) . ';';
            ...
            $classes[bname($this->pathService)] = str_replace(bname($classes[$key]), ucfirst(bname($this->pathService)), $classes[$key]) . "::class";
...
trait ALL {
    ...
    public function pathScan($rootService = true)
    {
        ...
        $serviceName = bname($pathService);

        foreach ($pathAll as $pathKey => $pathVal) {
            $pathTemp = bname($pathVal);
            ...
            if ($pathTemp !== $serviceName && $pathTemp !== 'ALL' && !empty($filesTemp)) {
                ...
                $filesAll[bname($pathVal)] = $filesTemp;
            }
        }
        if (empty(Az::$app->params['smartClass'])) {
            $deleteItems = ZFileHelper::scanFiles($path . '/ALL');
            foreach ($deleteItems as $key => $item) {
                if (ZArrayHelper::keyExists(str_replace('.php', '', lcfirst(bname($item))), $filesAll)) {
                    continue;
                }
//                if (file_exists($item)) {
//                    if (str_replace('.php', '', strtolower(bname($item))) === 'App')
//                        continue;
//                    unlink($item);
//                }
            }
        ...
        foreach ($filesAll as $pathName => $files) {
            ...
            foreach ($files as $fileKey => $fileVal) {
                ...
                foreach ($class[$pathName] as $key1 => $value1) {
                    ...
                    $item[$pathName]->property[] = bname($value1);
                    $item[$pathName]->propertyVar[] = lcfirst(bname($value1));
    ...
    public function getServiceData($class)
    {
        ...
        $item->ns = str_replace(['/' . bname($class), '/'], ['', '\\'], $class);
