...
namespace zetsoft\service\payme\Click\click\models;
...
class BasicPaymentsErrors {
    ...
    public function request_check($request){
        ...
        $sign_string = md5(
            ...
            ($request[paramAction] == 1 ? $request['merchant_prepare_id'] : '') .
            $request['amount'] .
            $request[paramAction] .
            $request['sign_time']
        );
        ...
        if (!((int)$request[paramAction] == 0 || (int)$request[paramAction] == 1)) {
            // return response array-like
            return [
                'error' => -3,
