...
use zetsoft\former\eyuf\EyufNeedCountForm;
...
$action->title = Azl.'Запрос потребностей по количеству';
...
$model->configs->query = EyufRequest::find()
    ->where([
        'form' => EyufNeedCountForm::class
    ]);

//vdd(EyufRequest::find()->all());
            ...
            'value' => function ($model, $key, $index, ZKDataColumn $dataColumn) {
                return ZButtonWidget::widget([
                    'config' => [
                        ...
                        'url' => ZUrl::to('/eyuf/logics/moderator/need-count-create.aspx?id=' . ZArrayHelper::getValue($model, 'id'))
                    ],
                    'event' => [
            ...
            'value' => function ($model, $key, $index, ZKDataColumn $dataColumn) {
                return ZButtonWidget::widget([
                    'config' => [
                        ...
                        'url' => ZUrl::to('/eyuf/logics/moderator/need-count-index.aspx?id=' . ZArrayHelper::getValue($model, 'id'))
                    ],
                    'event' => [
                        'click' => 'event.preventDefault(); window.open(this.href, "_blank")',
                    ]
...
echo ZDynaWidget::widget([
    ...
    'config' => [
        ...
        'columnAfter' => false,
        'columnBefore' => false,
        'title' => 'Запрос потребностей по количеству',
