...
use zetsoft\former\reports\ReportsAcceptCourierForm;
...
class ReportsXolmat extends ZFrame
{
    ...
    public function courierReport()
    {
        ...
        foreach ($companies as $company) {
            ...
            $total_order = $orders
                ->whereIn('shop_shipment_id', $total_shipment->pluck('id'))
                ->sum('total_price');
            ...
            foreach ($user_couriers as $user_courier) {
                ...
                $sold_orders = $new_orders
                    ->whereIn('status_accept', [
                        ShopOrder::status_client['delivered'], ShopOrder::status_client['received']
                    ])
                    ->count();
                ...
                $transfer = $new_orders
                    ->whereIn('status_accept', ['delivery_transfer'])
                    ->count();

                $shipment_price = $new_orders->where('shop_order_id', $user_courier['id'])->sum('total_price');

                $additional_delivery_price = $new_shipments
                    ->where('shop_order_id', $user_courier['id'])
                    ->sum('additional_received_money');
    ...
    public function getAcceptanceFromCourier($company_id = null)
    {
        ...
        if (!$this->wareAccept->isEmpty()) {
            ...
            foreach ($this->wareAccept as $item) {
                ...
                $shop_order = $this->orders
                    ->where('shop_shipment_id', $item['shop_shipment_id'])
                    ->where('status_accept', ShopOrder::status_accept['completed']);

                if ($company_id === null)
                    $shop_order_item = $this->order_items
                        ->whereIn('shop_order_id', $shop_order->pluck('id'));
                else
                    $shop_order_item = $this->order_items
                        ->whereIn('shop_order_id', $shop_order->pluck('id'))
                ...
                if (!$shop_order_item->isEmpty()) {
                    ...
                    foreach ($shop_order_item as $val) {
                        ...
                        // status static
                        $form->status = ShopOrder::status_accept['completed'];
    ...
    public function getAcceptanceFromCouries($from, $to, $status, $company_id = null)
    {

        $return = [];


        $wareAccepts = $this->wareAccept->whereBetween('created_at', [$from, $to]);

        if($wareAccepts->isEmpty())
            return [];
                      
        foreach ($wareAccepts as $wareAccept) {

            $shop_order = $this->orders
                ->where('shop_shipment_id', $wareAccept['shop_shipment_id'])
                ->where('status_accept', $status);

            if ($company_id === null)
                $shop_order_items = $this->order_items
                    ->whereIn('shop_order_id', $shop_order->pluck('id'));
            else
                $shop_order_items = $this->order_items
                    ->whereIn('shop_order_id', $shop_order->pluck('id'))
                ...
                if($shop_order_items->isEmpty())
                    return [];

                foreach ($shop_order_items as $shop_order_item) {

                    $form = new ReportsAcceptCourierForm();

                    $form->ware_accept_name = $wareAccept['name'] . $wareAccept['created_at'];
                    
                    $form->shop_order_name = $shop_order_item['created_at'];

                    $form->id = $shop_order_item['id'];

                    $form->amount = $shop_order_item['amount'];

                    $catalogName = $this->catalogs
                        ->where('id', $shop_order_item['shop_catalog_id'])
                        ->first();

                    $form->shop_catalog_name = $catalogName['name'] ?? '';

                    $form->client =  $shop_order_item['name'];
