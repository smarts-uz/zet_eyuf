    ...
    private function handleQuery(ConnectionInterface $from, $query)
    {
        switch ($query->target) {
            ...
            case 'USERS':
            {
                ...
                usort($fellowDtos, function ($fellow1, $fellow2) use ($userId) {
                    $lastMessagedAt1 = ChatMessage::find()
                        ->where(['sender' => $userId, 'receiver' => $fellow1->id])
                        ->orWhere(['sender' => $fellow1->id, 'receiver' => $userId])
                        ->orderBy('created_at')
                        ->max('created_at');

                    $lastMessagedAt2 = ChatMessage::find()
                        ->where(['sender' => $userId, 'receiver' => $fellow2->id])
                        ->orWhere(['sender' => $fellow2->id, 'receiver' => $userId])
                        ->orderBy('created_at')
                        ->max('created_at');

                    if ($lastMessagedAt1 == null && $lastMessagedAt2 == null) {
                        return 0;
                    } else if ($lastMessagedAt1 == null) {
                        return -1;
                    } else if ($lastMessagedAt2 == null) {
                        return 1;
                    }

                    return strtotime($lastMessagedAt2) - strtotime($lastMessagedAt1);
                });
