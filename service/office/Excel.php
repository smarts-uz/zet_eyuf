...
class Excel extends ZFrame
{
    ...
    public function run()
    {
        ...
        foreach ($attributeLabels as $headColName => $headColLabel) {
            ...
            $activeSheetIndex->setCellValue($pCoord . '1', $headColName);
            //set background of cell to #22272e
            $spreadsheet->getActiveSheet()->getStyle($pCoord . '1')->getFill()->setFillType(\PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID)->getStartColor()->setARGB('22272e');

            $activeSheetIndex->setCellValue($pCoord . '2', $headColLabel);
