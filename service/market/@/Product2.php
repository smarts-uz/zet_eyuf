...
#region test collection
    ...
    {
        $category = ShopCategory::findOne(550);
        ...
        $brands = [];
        $all_brand = ShopBrand::find()->all();
        $all_brand = collect($all_brand);
        foreach ($products as $product) {
            $brand = $all_brand->where('id', $product->id);
            $brands[] = $brand;
        }
...
#region init
    ...
    public function init()
    {
        ...
        $this->core_option_types = collect(ShopOptionType::find()->all());
        $this->core_options = collect(ShopOption::find()->all());
        $this->core_option_type_branches = collect(ShopOptionBranch::find()->all());
        parent::init();
    }
...
#region productItem and database
    ...
    public function product($shop_product_id = null, $company_id = null)
    {
        ...
        $shop_product = ShopProduct::findOne($shop_product_id);

        if (!$shop_product) return null;

        $core_category = ShopCategory::findOne($shop_product->shop_category_id);
        ...
        foreach ($shop_catalogs as $core_catalog) {
            ...
            if (is_array($catalog_offers)) {
                foreach ($catalog_offers as $catalog_offer) {
                    ...
                    if (time() < $end)
                        $offers[] = ZProductItem::statuses['sale'];
                }
            }
        }
    ...
    {
        $shop_element = ShopElement::findOne($element_id);
        ...
        if (\Dash\count($shop_element->shop_option_ids))
            foreach ($shop_element->shop_option_ids as $option_id) {
                $option = ShopOption::findOne($option_id);
                $option_type = ShopOptionType::findOne($option->shop_option_type_id);
        ...
        if (!is_array($shop_product->shop_option_ids)) {
            ...
            $not_combination_option_ids = array_filter($shop_product->shop_option_ids, function ($a) use ($core_category) {
                $core_option = ShopOption::findOne($a);
                $core_option_type = ShopOptionType::find()
                    ->where([
    ...
    public function getPropertsByCategory2($category_id = null, $is_combination = false)
    {
        ...
        $core_option_types = ShopOptionType::find()->all();
    ...
    {
        if ($category_id == null) {
            $branches = ShopOptionBranch::find()->all();
        } else {
            ...
            $branch_ids = array_map(function ($a) {
                if (array_key_exists('shop_option_type', $a))
                    if ($a["shop_option_type"] !== "")
                        $core_option_type = ShopOptionType::findOne($a["shop_option_type"]);
            ...
            $branches = ShopOptionBranch::find()
                ->where([
                    'id' => $branch_ids
                ])->all();
        }
    ...
    public function propertsByCategory($category_id = null, $is_combination = false, $branch_id = null)
    {
        ...
        foreach ($core_option_types as $core_option_type) {
            $property_item = new PropertyItem();
            $property_item->name = $core_option_type->name;
            $branch = ShopOptionBranch::findOne($core_option_type->shop_option_branch_id);
            $property_item->branch = $branch->name;

            $core_option = ShopOption::find()->where([
                'shop_option_type_id' => $core_option_type->id
            ])->all();
...
#region cart
    ...
    public function catalogsByElement($productId, $options = [])
    {
        ...
        $shop_product = ShopProduct::findOne($productId);
...
//        if(!isset($element))
    ...
    public function saveElements($model)
    {
        ...
        $combination_core_option_types = ShopOptionType::find()
            ->where([
                'id' => $core_option_type_ids,
            ])
            ->all();
