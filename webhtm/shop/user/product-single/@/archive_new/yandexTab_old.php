...
$tabItems = [];
$tabItem = new TabItem();
$tabItem->url = '/shop/user/product-single/common.aspx?id=' . $product_id;
...
$tabItem = new TabItem();
$tabItem->url = '/shop/user/product-single/map.aspx?id=' . $countQuestion;
    ...
    <ul class="nav d-flex text-center justify-content-between" role="tablist">
        <?php foreach ($tabItems as $tabItem) : ?>
            ...
            <li class="nav-item border  px-0 col-sm">
                <a class="nav-link zLoader waves-light text-muted h-100 w-100 fp-17 show <?= $tabItem->active ? 'active' : '' ?>"
                   id="href-<?= $tabItem->id ?>"
                   href="<?= $tabItem->url ?>"
                >
                    <?= $tabItem->title ?>
