...
class Productold extends ZFrame
{
...
#region for AdminPanel
    ...
    public function getOptionsByCategory($selectedId = null)
    {
        ...
        if (isset($selectedId)) {
            if (!empty($selectedId)) {
                $category = ShopCategory::findOne($selectedId);
                $option_types = $category->shop_option_type;
                if (empty($option_types))
                    return null;
                $data = [];
                foreach ($option_types as $option_type) {
                    $core_option_type = ShopOptionType::findOne((int)$option_type['shop_option_type']);
                    if (empty($core_option_type)) {
                        return null;
                    }
...
#region productItem and database
    ...
    {
        $shop_products = array_filter($shop_products, function ($shop_product) {
            ...
            if ($shop_catalogs === null or $shop_catalogs == [])
                return false;
            return true;
        });
...
//        if(!isset($element))
   ...
   /* public function testSaveElements() {
        $model = CoreProduct::findOne(12);
        $this->saveElements($model);
    }*/
