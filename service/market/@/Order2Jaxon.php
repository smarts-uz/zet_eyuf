...
class Order2Jaxon extends ZFrame
{
    /**
    ...
    {    //#Hammasi ishliydi tekshirilgan
        //$this->getOrderList(1, 'new');
        //vdd($this->getUserOrderList());
        //$this->getUserOrderList('new');
        //$this->getProductBelongsToOrder(15);// Tekshirilgan ishliydi
        //$this->getOrderAddress(); // idsi bor obyekt ketishi kerak
    ...
    public function getProductBelongsToOrder($order_id = null)
    {
        ...
        $current_order = $this->core_orders->where('id', $order_id)->first();
        //$order_items = $this->core_order_items->whereIn('id', $current_order->core_order_item_ids);
          $current_order = $this->toObject(ShopOrder::class, $current_order);

        $current_order->core_order_item_ids=json_decode($current_order->core_order_item_ids,true);
        $order_items = $this->core_order_items->whereIn('id', $current_order->core_order_item_ids);

        foreach ($order_items as $order_item){
            $order_item = $this->toObject(ShopOrderItem::class, $order_item);
