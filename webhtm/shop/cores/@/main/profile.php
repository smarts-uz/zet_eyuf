...
use zetsoft\models\core\GovsSpeciality;
...
<div id="block_users">
    <div class="left">
        ...
        $user = User::findOne(1);
        $special = GovsSpeciality::find()->where(['id' => $user['govs_speciality_ids']])->one();
        $country = PlaceCountry::find()->where(['id' => $user['place_country_id']])->one();
        $imgPath = $user['id'];
        $img = $user['photo'];
        //$imgg = implode($img);
