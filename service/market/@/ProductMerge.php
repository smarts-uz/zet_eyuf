 ...
 * Refactored by: Xolmat , Umid Berdiyev
 * Date:    07.06.2019
...
class ProductMerge extends ZFrame
{
    // till 1156
    /**
...
#region test collection
    ...
    private function test1()
    {
        ...
        //vdd($this->getOptionsByCategory(40));
        // vdd($this->getCatelogsByProductId(40));
        //vdd($this->getBranchesByCategory('125'));
        //vdd($this->propertsByCategory('125'));
        // vdd($this->catalogsByElement(32));
        // vdd($this->allProducts(549));
        // vdd($this->getAllProductsByBrand(21));
        // vdd($this->getAllProductsByCompany(null, 40));
        //vdd($this->allCompanies());
        //vdd($this->product(19, 2));
        //  vdd($this->singleProductItemByOptions(19,41));
        //   vdd($this->productItemByElementId(19,41));
        //  vdd($this->getPropertsByCategory2(40));
        // vdd($this->getBranchesByCategory(549));
        //  vdd($this->getOptionsByCategory(549));
...
#region init
    ...
    public function init()
    {
        ...
        /*       $this->shop_products = collect(CoreProduct::find()
               ...
               $this->shop_elements = collect(CoreElement::find()->asArray()->all());
               $this->shop_catalogs = collect(ShopCatalog::find()->asArray()->all());
               $this->shop_categories = collect(CoreCategory::find()->asArray()->all());
               $this->core_option_types = collect(CoreOptionType::find()->asArray()->all());
               $this->core_options = collect(CoreOption::find()->asArray()->all());
               $this->core_companies = collect(UserCompany::find()->asArray()->all());
               $this->core_option_type_branches = collect(CoreOptionTypeBranch::find()->asArray()->all());*/
    ...
    public function getOptionsByCategory($selectedId = null)
    {
        ...
        if (isset($selectedId)) {
            if (!empty($selectedId)) {
                ...
                $category = $this->toObject(ShopCategory::class, $category);
                ...
                foreach ($option_types as $option_type) {
                    ...
                    /*$core_option_type = $this->toObject(CoreOptionType::class, $core_option_type,true);*/
                    #check
                    $options = $this->core_options
                        ->where('shop_option_type_id', $core_option_type->first()->id);
...
#region productItem and database
    ...
    public function allProducts($category_id = null, $status = null, $page = null, $limit = null)
    {
        ...
        if ($page !== null and $limit !== null) {
            $shop_products = $this->shop_products->skip($page * $limit)->take($limit);
        } else {
        ...
        foreach ($shop_products as $shop_product) {
            $shop_product = $this->toObject(ShopProduct::class, $shop_product);
    ...
    public function allCompanies()
    {
        ...
        foreach ($companies as $company) {
            $company = $this->toObject(UserCompany::class, $company);
...
//////////////////////////////////////
        $item = $this->catalog_id_or_elements($item, $company_id, $elements);
        ...
        $item = $this->properties($item, $category_properties, $shop_product);
        $item = $this->allProperties($item, $core_category, $shop_product);
        $item = $this->image($item, $shop_product);
        $item = $this->amount_price($item, $elements);
        
        /////////////////
        return $item;
    }

    private function catalog_id_or_elements($item, $company_id, $elements)
    {
        if ($company_id === null) {
            ...
            if ($company_id !== null) {
                if (!$this->is_multi($core_category)) {
                    ...
                    $a = $this->toObject(ShopElement::class, $elements->first());
                    $catalog = $this->shop_catalogs
                        ->where('shop_element_id', $a->id)->where('user_company_id', $company_id)
                        ->first();
    ...
    private function properties($item, $category_properties, $shop_product)
    {
        $properties = [];
        foreach ($category_properties as $key1 => $category_property) {
            //vdd(gettype($shop_product->shop_option_ids));
            $items = [];
    ...
    private function allProperties($item, $core_category, $shop_product)
    {
        //All properties
    ...
    private function image($item, $shop_product)
    {
        $item->images = [];
    ...
    private function amount_price($item, $elements)
    {
        ...
        foreach ($elements as $element) {
            ...
            foreach ($catalogs as $catalog) {
                $catalog = $this->toObject(ShopCatalog::class, $catalog);
    ...
    public function productOffers($shop_product_id, $company_id = null)
    {
        ...
        $shop_element_ids = $shop_elements->pluck("id");
        // $shop_element_ids = json_decode($shop_element_ids);
        if ($company_id === null) {
        ...
        foreach ($shop_catalogs as $core_catalog) {
            $core_catalog = $this->toObject(ShopCatalog::class, $core_catalog);
    ...
    public function singleProductItemByOptions($product_id, $company_id, $options = [])
    {
        ...
        foreach ($elements as $element) {
            $element = $this->toObject(ShopElement::class, $element);
            $element->shop_option_ids = json_decode($element->shop_option_ids);
            $bool = true;
    ...
    public function productItemByElementId($element_id, $user_company_id = null)
    {
        ...
        #check 
        if ($shop_element !== null)
            $shop_element = $this->toObject(ShopElement::class, $shop_element);
            ...
            if ($shop_element->shop_option_ids !== null)
                foreach ($shop_element->shop_option_ids as $option_id) {
                    ...
                    $option_type = $this->core_option_types
                        ...
                        ->first();
                    $option_type = $this->toObject(ShopOptionType::class, $option_type);
        ...
        if (!is_array($shop_product->shop_option_ids)) {
            ...
            $not_combination_option_ids = array_filter($shop_product->shop_option_ids, function ($a) use ($core_category) {
                ...
                $core_option = $this->core_options
                    ->where('id', $a)
                    ->first();
                $core_option = $this->toObject(ShopOption::class, $core_option);
                /* $core_option_type = CoreOptionType::find()
                     ->where([
                         'id' => $core_option->shop_option_type_id
                     ])->one();*/
    ...
    public function getPropertsByCategory2($category_id = null, $is_combination = false)
    {
        ...
        if ($category_id) {
            ...
            $core_category->shop_option_type = json_decode($core_category->shop_option_type);
            /*
    ...
    public function getBranchesByCategory($category_id = null)
    {
        if ($category_id == null) {
            ...
            if ($category === null)
                return false;
            $category = $this->toObject(ShopCategory::class, $category);
            $category->shop_option_type = json_decode($category->shop_option_type);
    ...
    public function propertsByCategory($category_id = null, $is_combination = false, $branch_id = null)
    {
        ...
        if ($branch_id !== null)
            ...
            $core_option_types = $core_option_types->filter(function ($a) use ($branch_id) {
                $a = $this->toObject(ShopOptionType::class, $a);
                if ($a->shop_option_branch_id === $branch_id)
                    return true;
                else
                    return false;
    ...
    public function is_multi($core_category)
    {
        ...
        $option_type_json = json_decode($option_type_json);
        foreach ($option_type_json as $option_type) {
...
//        AZ::$app->utility->monolog->log('asdf');
    ...
    public function getCatelogsByProductId($shop_product_id)
    {
        ...
        $shop_element_ids = $shop_elements->map(function ($a) {
            $a = $this->toObject(ShopElement::class, $a);
            return $a->id;
        });
    ...
    public function filterFormItems($category_id = null)
    {
        ...
        if (!empty($products)) {
            ...
            $product_ids = $products->map(function ($a) {
                $a = $this->toObject(ShopProduct::class, $a);
                return $a->id;
            })->unique();
            ...
            $catalog_prices = $shop_catalogs->map(function ($a) use ($currency) {
                $a = $this->toObject(ShopCatalog::class, $a);
                return Az::$app->payer->currency2->convert($a->currency, $currency, $a->price);
            });
