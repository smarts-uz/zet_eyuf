...
//        'Control'
    ...
    {
        foreach ($this->exceptStrings as $items) {
            if (ZStringHelper::find($message, $items))
                return true;
        }
    }
    #endregion
    ...
    public function filePerDayLog($message, $level = null)
    {
        ...
        if (!is_writable($this->logFilePerDay['file'])){
            return null;
        }
        ...
        if ($traceLevel > 0) {
            ...
            foreach ($ts as $trace) {
                if (
                    ...
                    ZStringHelper::find($trace['file'], YII2_PATH) !== 0 &&
                    ZStringHelper::find($trace['file'], Monolog::class) === false &&
                    ZStringHelper::find($trace['file'], 'vendor\monolog\monolog\src') === false &&
                    ZStringHelper::find($trace['file'], 'system\Az') === false &&
                    ZStringHelper::find($trace['file'], 'system\monolog\\') === false
                ) {
    ...
    private function isExcept()
    {
        ...
        foreach ($trace as $itemKey => $itemValue) {
            ...
            if (isset($itemValue['function']) && ZArrayHelper::isIn($itemValue['function'], $this->commands))
