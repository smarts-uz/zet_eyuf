 ...
 */
class CategoryXolmat extends ZFrame
{
    public $categories;
    public $brands;
    ...
    {
        $this->categories = collect(ShopCategory::find()->indexBy('id')
            ->asArray()
            ->all());
        parent::init();
    }
    ...
    public function getBrands()
    {
        ...
        if (!empty($selectedId)) {
            ...
            $data = $category->getShopBrandsFromShopBrandIds();
            $brands = [];
            if (empty($data))
                return null;
    ...
    public function generateCatalogName(ShopCatalog $model)
    {
        ...
        if ($a === false) {
            ...
            $catalog = ShopCatalog::findOne($model->shop_element_id);
            

            $model->name = $element->name.'-'.$catalog->price;
        ...
        $this->user_company_id = $this->userIdentity()->user_company_id;
